...
def analyze_conceptual_wing(wing: ConceptualWing, N: int = 8, M: int = 4, rho: float = 0.00231) -> ConceptualWing:
    """
...
def analyze_wing_vlm(
    ...
    for alpha_deg in alpha_degrees:
        ...
        CL, CDi, CM, L,Di,M = _calculate_coefficients(
...
def plot_wing_analysis(wing: ConceptualWing, save_plots: bool = False):
    ...
    ax1.plot(results.alpha_degrees, results.L, 'bo-', linewidth=2, markersize=4, label='L')
    ...
    plt.suptitle(f'Complete Wing Analysis: {wing.airfoil.name}\n'
                 ...
                 f'V = {wing.airspeed} ft/s', fontsize=14)
    plt.tight_layout()
...
#####################################################################
    ...
    wing = ConceptualWing(
        ...
        aoa_max=12, # deg
        aoa_diff=1.0, # deg
        airspeed= 55 * 1.466666667 # input mph converts to ft/s
    )
