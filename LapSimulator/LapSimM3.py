...
def xflr_results(interpolators,name,value, banner_length, AR):
    name = name.lower()  # Normalize to lowercase for easier matching

    if name == "cl":
        k_orig = 1 / (np.pi * 5.17 * 0.9)  # Induced drag factor with efficiency factor of 0.9
        CD0_orig = (interpolators['cd_of_cl'](value)) - (k_orig * value**2)
        if CD0_orig < 0:
            CD0 = 0.02
        else:
            CD0 = CD0_orig * (banner_length / 12)
        CDI = (k_orig * value**2) * (5.17 / AR)
        CD = CD0 + CDI
        ...
        k_orig = 1 / (np.pi * 5.17 * 0.9)  # Induced drag factor with efficiency factor of 0.9
        CD0_orig = interpolators['cd_of_alpha'](value) - (k_orig * CL**2)
        if CD0_orig < 0:
            CD0 = 0.02
        else:
            CD0 = CD0_orig * (banner_length / 12)
        CDI = (k_orig * CL**2) * (5.17 / AR)
        CD = CD0 + CDI
...
def straight(state:dict, constants:dict, distance_needed, batt_interp, thrust_interp, coeff_interp):
    ...
    rho = constants['rho']
    dt = constants['dt']
    AR = constants['wing_span']**2 / S  # Aspect Ratio
    ...
    while distance_needed >= distance_traveled:
        ...
        CD_Needed,alpha = xflr_results(coeff_interp, "Cl", CL_Needed, constants['banner_length'], AR) # Get CL and CD at 0 deg alpha
...
def expose_vector(constants):
    ...
    AR = constants['wing_span']**2 / constants['S']  # Aspect Ratio

    # Calculate max/cruise aoa based on weight and XFLR data
    cl_max,cd_max = xflr_results(coeff_interp, 'alpha', constants['max_aoa'], constants['banner_length'], AR)
    ...
    cruise_cd, cruise_aoa = xflr_results(coeff_interp, 'Cl', cruise_cl, constants['banner_length'], AR)
    constants['cruise_coeffs'] = [cruise_aoa, cruise_cl, cruise_cd]

    state = {
        'velocity': np.array([[82.0,0.0]]), #ft/s
        'position': np.array([[constants['altitude'],0.0]]), #ft
        'acceleration': np.array([[0.0,0.0]]), #ft/s^2
        'battery_charge': np.array([constants['battery_capacity']]), #mAh
        'time': np.array([15]), #seconds
    ...
    print(f"Inputs:")
    print(f"Banner Length: {constants['banner_length']} in")
    print(f"Wing Span: {constants['wing_span']} ft")
    print(f"Empty Weight: {constants['W']} lbs")
    print(f"Battery Remaining: {state['battery_charge'][-1]}")
    print(f"Final Speed: {state['velocity'][-1,0]} ft/s")
...
if __name__ == '__main__':
    ...
    for lap in range(constants['lap_count']):
        ...
        turn(state,constants,180,batt_interp,thrust_interp,coeff_interp)
        straight(state,constants,500,batt_interp,thrust_interp,coeff_interp)
        print(f'Lap {lap + 1} complete, total timesteps: {state["i"]}')
